...
import sit.int221.kanbanapi.services.*;
                ...
                if (requestTokenHeader != null && requestTokenHeader.startsWith("Bearer ")) {
                    ...
                    try {
                        if (!jwtTokenUtil.isTokenItbkk(jwtToken)) {
                            MsUser msUser = jwtTokenUtil.getUserFromMicrosoftToken(jwtToken);
                            msUser = userService.msUser(msUser);
                            userOid = msUser.getId();
                            userDetails = jwtUserDetailsService.loadUserByOid(userOid);
                        } else {
                            ...
                            userOid = claims.get("oid", String.class);
                            userDetails = jwtUserDetailsService.loadUserByOid(userOid);
                        }
